...
if(!global.isGamePaused) //only execute when not paused
{
    ...
    if(distance > 350)
    {
        physics_apply_local_force(0,0, 4, 0);
    }
    else
    {
    ...
    if(life <= 0)
    {
      ...
      var collision = point_distance(x, y, global.planetInstance.x, global.planetInstance.y);
        if(collision <= 170)
        {
            instance_destroy();
           global.planetInstance.life -= ENEMY_SHIP_PROJECTILE_BASEDAMAGE * 2;
    }
